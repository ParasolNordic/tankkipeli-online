...
io.on('connection', (socket) => {
    console.log('🟢 Pelaaja liittyi:', socket.id);
    
    // Lähetä palvelimen tila (onko hereillä)
    socket.emit('serverAwake', { 
        awake: true, 
        timestamp: Date.now(),
        message: 'Palvelin on valmis!'
    });
    ...
    socket.on('createGame', ({ playerCount, bounceEnabled }) => {
        ...
        console.log(`🎮 Peli luotu: ${roomCode} (${playerCount} pelaajaa, bounce: ${bounceEnabled})`);
    ...
    socket.on('joinGame', (roomCode) => {
        ...
        if (!game) {
            socket.emit('error', { message: 'Peliä ei löytynyt koodilla: ' + roomCode });
            return;
        }
        ...
        console.log(`👥 Pelaaja ${playerNumber} liittyi peliin ${roomCode}`);
    ...
    socket.on('disconnect', () => {
        ...
        if (socket.id === game.hostId || Object.keys(game.players).length === 0) {
            console.log(`🗑️ Peli poistettu: ${roomCode}`);
...
🎮 ════════════════════════════════════════
   ...
   ⏰ Käynnistyaika: ${new Date().toLocaleString('fi-FI')}
🎮 ════════════════════════════════════════
    `);
});
...
setInterval(() => {
    ...
    games.forEach((game, roomCode) => {
        if (now - game.lastUpdate > TIMEOUT) {
            console.log(`🧹 Siivotaan vanha peli: ${roomCode}`);
            games.delete(roomCode);
            roomCodes.delete(roomCode);
        }
    });
